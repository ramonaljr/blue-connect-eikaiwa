...
import { getConnectedProviders } from '@/lib/actions/settings'
...
const PROVIDERS = [
  ...
  { key: 'line', name: 'LINE', icon: LineIcon },
] as const
...
export function ConnectedAccountsSection({ user: _user }: { user: User }) {
  ...
  function getConnectedDate(providerKey: string) {
    const provider = providers.find((p) => p.provider === providerKey)
    if (!provider?.created_at) return null
    return new Date(provider.created_at).toLocaleDateString('ja-JP')
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {loading ? (
          ...
          PROVIDERS.map(({ key, name, icon: Icon }) => {
            const connected = isConnected(key)
            const connectedDate = getConnectedDate(key)
